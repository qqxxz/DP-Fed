...
def create_heterogeneous_emnist_datasets(
    alpha: float = 1.0,
    num_clients: int = 100,
    batch_size: int = 32,
    test_batch_size: int = 1024
) -> Tuple[Dict[str, Dataset], DataLoader]:
    """Create heterogeneous EMNIST datasets using Dirichlet distribution.
    ...
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.1751,), (0.3333,))  # EMNIST 数据集的归一化参数
    ])
