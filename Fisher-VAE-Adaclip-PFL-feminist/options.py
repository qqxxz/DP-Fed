...
def parse_args():
    ...
    parser.add_argument('--fisher_threshold', type=float, default=0.2, help="Fisher information threshold for parameter selection")
    parser.add_argument('--lambda_1', type=float, default=0.05, help="Lambda value for EWC regularization term")
    ...
    parser.add_argument('--user_sample_rate', type=float, default=1)####可选（随机选择的客户端用于训练的比例）
    ...
    # --------------------------------差分隐私设置--------------------------------
    parser.add_argument('--target_epsilon', type=float, default=8, help="Target privacy budget epsilon")
    ...
    parser.add_argument('--noise_multiplier', type=float, default=0.9)##可选噪声乘子为：{0, 0.01, 0.03, 0.1}
    parser.add_argument('--max_clip_threshold', type=float, default=1.5, help='最大裁剪阈值，0表示不设限制')
    ...
    parser.add_argument('--UPDATE_SCALING_FACTOR', type=float, default=0.1)
    # --------------------------------任务设置与运行模式--------------------------------
